module.exports = function(RED) {
    ...
    function filter_ui(config) {
        RED.nodes.createNode(this, config);
        var node = this;

        node.on('input', function(msg) {
            var data = json_filter(JSON.parse(msg.payload), config.attribute, config.operator, config.value);
            msg.payload = JSON.stringify(data);
            node.send(msg);
        });
    }

    RED.nodes.registerType("filter_ui", filter_ui);
}