module.exports = function(RED) {
    ...
    //Group
    var json_categorize = function(data, attribute) {
        var exec_query = null;
    ...
    function categorize(config) {
        RED.nodes.createNode(this, config);
        var node = this;

        node.on('input', function(msg) {
            var data = json_categorize(JSON.parse(msg.payload.data), msg.payload.attribute);
            msg.payload = JSON.stringify(data);
            node.send(msg);
        });
    }

    RED.nodes.registerType("categorize", categorize);
}