module.exports = function(RED) {
    ...
    function custom_output_ui(config) {
        RED.nodes.createNode(this, config);
        var node = this;

        node.on('input', function(msg) {
            var data = json_output_custom(JSON.parse(msg.payload), config.attribute, config.types);
            msg.payload = JSON.stringify(data);
            node.send(msg);
        });
    }

    RED.nodes.registerType("custom_output_ui", custom_output_ui);
}